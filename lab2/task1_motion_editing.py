...
def part1_translation_and_rotation(viewer, setting_id):
    ...
    # 一些不同的设置
    bvh_list = [
        "lab2/motion_material/walk_forward.bvh",
        "lab2/motion_material/run_forward.bvh",
        "lab2/motion_material/walk_and_turn_left.bvh",
    ]
...
def part2_interpolate(viewer, v):
    ...
    # 调整方向和位置, 对齐第一帧
    walk_forward = walk_forward.translation_and_rotation(
        0, np.array([0, 0]), np.array([0, 1])
    )
    run_forward = run_forward.translation_and_rotation(
        0, np.array([0, 0]), np.array([0, 1])
    )
    ...
    distance = (1 - blend_weight) * walk_forward.joint_position[
        -1, 0, 2
